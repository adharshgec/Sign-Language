...
def create_empty_images(folder_name, n_images):
	...
	black = np.zeros(shape=(image_height, image_width, 1), dtype=np.uint8)
...
def store_images(g_id):
	...
	while True:
		...
		dst = cv2.calcBackProject([imgHSV], [0, 1], histogram, [0, 180, 0, 256], 1)
		...
		if len(contours) > 0:
			...
			if cv2.contourArea(contour) > 10000 and frames > 50:
				...
				save_img = cv2.resize(save_img, (image_height, image_width))
